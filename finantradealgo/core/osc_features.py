...
class OscFeatureConfig:
    """
    Oscillator / momentum feature ayarları.
    """
...
def _add_rsi(df: pd.DataFrame, cfg: OscFeatureConfig) -> None:
    """
    Basit RSI (rolling mean) – Wilder tam aynısı değil ama iş görür.
    """
    close = df["close"]
    delta = close.diff()
...
def _add_macd(df: pd.DataFrame, cfg: OscFeatureConfig) -> None:
    ...
    df[f"macd_signal_{cfg.macd_fast}_{cfg.macd_slow}_{cfg.macd_signal}"] = macd_signal
    df[f"macd_hist_{cfg.macd_fast}_{cfg.macd_slow}_{cfg.macd_signal}"] = macd_hist
...
def add_osc_features(
    ...
    """
    RSI, Stoch, MACD, CCI, MFI gibi oscillator feature'larını ekler.
    """
    if config is None:
