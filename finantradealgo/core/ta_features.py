...
class TAFeatureConfig:
    """
    OHLCV tabanlı teknik indikatör ayarları.

    Not: Şimdilik tüm periodler default; istersen ileride config üzerinden oynarsın.
    """
    # Return & momentum
    ...
    hv_periods: tuple[int, ...] = (10, 20, 50)  # raw std; annualize etmedik

    # Bollinger Band
    bb_period: int = 20
    bb_std: float = 2.0

    # Support / resistance rolling window
...
def _add_hv(df: pd.DataFrame, periods: Iterable[int]) -> None:
    ...
    for n in periods:
        ...
        # İleride istersen annualize: * np.sqrt(365 * bar_per_day)
...
def _add_bollinger(df: pd.DataFrame, period: int, std_mult: float) -> None:
    ...
    # Band genişliği: normalize close’a göre
...
def _add_support_resistance(df: pd.DataFrame, periods: Iterable[int]) -> None:
    ...
    for n in periods:
        ...
        # Fiyat bandın neresinde? 0: dip, 1: tepe
...
def add_ta_features(df: pd.DataFrame, config: TAFeatureConfig | None = None) -> pd.DataFrame:
    """
    OHLCV tabanlı temel teknik analiz feature'ları ekler.

    Giriş:
        df: open, high, low, close, volume kolonlarını içeren DataFrame.
    Çıkış:
        Aynı DataFrame (kopya) üzerinde yeni sütunlar eklenmiş hali.
    """
    if config is None:
    ...
    # 1) Returns ve momentum
    _add_returns(df, config.ret_periods)
    _add_momentum(df, config.mom_periods)

    # 2) EMA ve basit trend flag'leri
    _add_ema(df, config.ema_periods)

    # 3) ATR ve volatilite yüzdesi
    _add_atr(df, config.atr_periods)

    # 4) Historical volatility (log-return std)
    _add_hv(df, config.hv_periods)

    # 5) Bollinger band ve band genişliği
    _add_bollinger(df, config.bb_period, config.bb_std)

    # 6) Support / resistance rolling high/low ve band içi pozisyon
