...
def main() -> None:
    ...
    run_id = f"portfolio_{portfolio_cfg.timeframe}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
    out_dir = Path("outputs")
    bt_dir = out_dir / "backtests"
    tr_dir = out_dir / "trades"
    bt_dir.mkdir(parents=True, exist_ok=True)
    tr_dir.mkdir(parents=True, exist_ok=True)

    equity_df = pd.DataFrame({"time": result.portfolio_equity.index, "portfolio_equity": result.portfolio_equity.values})
    equity_path = bt_dir / f"{run_id}_equity.csv"
    equity_df.to_csv(equity_path, index=False)

    for sym, eq in result.symbol_equity.items():
        sym_path = bt_dir / f"{run_id}_equity_{sym}.csv"
        pd.DataFrame({"time": eq.index, sym: eq.values}).to_csv(sym_path, index=False)

    trades_path = tr_dir / f"{run_id}_trades.csv"
    result.trades.to_csv(trades_path, index=False)

    print(f"[INFO] Portfolio backtest saved to {equity_path} and {trades_path}")
    if result.metrics:
        print("[INFO] Metrics:", result.metrics)
